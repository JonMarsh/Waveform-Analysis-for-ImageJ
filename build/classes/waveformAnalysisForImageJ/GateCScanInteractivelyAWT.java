 ...
 * @version 2014-09-24
 */
public class GateCScanInteractivelyAWT implements PlugIn
{
	...
	static GateCScanInteractivelyAWT instance;
	...
	private class CreateGatesForCScanDialog extends ImageWindow implements ActionListener, ImageListener
	{
		...
		GateCScanInteractivelyAWTControlPanel panel;
		...
		{
			panel = new GateCScanInteractivelyAWTControlPanel();
			...
			for (int imageID : suitableImageIDs) {
				panel.gateApplicationChoice.add(WindowManager.getImage(imageID).getTitle());
			}
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if (source == panel.okButton) {
				if (gatesExist) {
					...
					int selectedImageID = suitableImageIDs[panel.gateApplicationChoice.getSelectedIndex()]; // apply gating to the desired image (not necessarily the image the gates were computed with)
					short[] gatePositions = (short[])gateProcessor.getPixels();
		...
		public void imageClosed(ImagePlus image)
		{
			if (image.getID() == inputImageID) {
				...
				suitableImageIDs = getSuitableImageIDs(inputImage, gateImage);
				panel.gateApplicationChoice.removeAll();
				for (int imageID : suitableImageIDs) {
					panel.gateApplicationChoice.add(WindowManager.getImage(imageID).getTitle());
				}
			}
